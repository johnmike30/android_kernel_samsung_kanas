...
{
	hwspinlock_vid = 0;
	__raw_writel(BIT_SPINLOCK_EB, (void __iomem *)REG_GLB_SET(REG_AHB_AHB_CTL0));
	return 0;
}
...
static __inline __init int __hwspinlock_init(void)
{
	hwspinlock_vid = __raw_readl((void __iomem *)(SPRD_HWLOCK0_BASE + 0xffc));
	__raw_writel(BIT_SPINLOCK_EB, (void __iomem *)REG_GLB_SET(REG_AP_AHB_AHB_EB));
	__raw_writel(BIT_SPLK_EB, (void __iomem *)REG_GLB_SET(REG_AON_APB_APB_EB0));
	return 0;
}
#endif
...
static inline int arch_hwlock_fast_trylock(unsigned int lock_id)
{
	...
	if (hwspinlock_vid == 0x100) {
		if (!readl((void __iomem *)addr))
			goto __locked;
	} else if (hwspinlock_vid == 0) {
		if (HWSPINLOCK_NOTTAKEN_V0 == __raw_readl((void __iomem *)addr)) {
			__raw_writel(HWSPINLOCK_WRITE_KEY, (void __iomem *)addr);
			if (HWSPINLOCK_WRITE_KEY == __raw_readl((void __iomem *)addr)) {
				goto __locked;
			}
		}
	}
...
static inline void arch_hwlock_fast_unlock(unsigned int lock_id)
{
	...
	if (hwspinlock_vid == 0x100)
		__raw_writel(HWSPINLOCK_NOTTAKEN_V1, (void __iomem *)addr);
	else
		__raw_writel(HWSPINLOCK_NOTTAKEN_V0, (void __iomem *)addr);
}
