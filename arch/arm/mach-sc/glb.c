...
{
	return __raw_readl((void __iomem *)reg) & msk;
}
...
int sci_glb_write(u32 reg, u32 val, u32 msk)
{
	...
	__raw_writel((__raw_readl((void __iomem *)reg) & ~msk) | val, (void __iomem *)reg);
...
{
	if (__is_glb(reg)) {
		__raw_writel(bit, (void __iomem *)REG_GLB_SET(reg));
	} else {
		unsigned long flags;
		__arch_default_lock(HWLOCK_GLB, &flags);
		__raw_writel(__raw_readl((void __iomem *)reg) | bit, (void __iomem *)reg);
		__arch_default_unlock(HWLOCK_GLB, &flags);
	}
	return 0;
}
...
{
	if (__is_glb(reg)) {
		__raw_writel(bit, (void __iomem *)REG_GLB_CLR(reg));
	} else {
		unsigned long flags;
		__arch_default_lock(HWLOCK_GLB, &flags);
		__raw_writel((__raw_readl((void __iomem *)reg) & ~bit), (void __iomem *)reg);
